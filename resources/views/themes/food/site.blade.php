...
<main>
    @foreach ($template['sections'] as $type => $sectionSchema)
        ...
        @include("themes.food.sections.{$type}", ['data' => $data, 'site' => $site])
    @endforeach
</main>

@include('themes.food._footer')
...
@if ($sections->has('whatsapp_cta') && $sections->get('whatsapp_cta')->active)
    ...
    @if (!empty($wa['phone']))
        <a class="wa-btn"
           href="https://wa.me/{{ preg_replace('/\D/', '', $wa['phone']) }}?text={{ urlencode($wa['message'] ?? '¡Hola! Quiero hacer una reserva.') }}"
           target="_blank"
