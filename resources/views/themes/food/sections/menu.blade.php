...
@elseif ($layout === 'numbered')

    <section id="menu" class="section-wrap section-light">
        <div class="section-inner">
            <div class="section-heading">
                <h2>Carta / Menú</h2>
                <div class="heading-line"></div>
            </div>
            <div class="menu-numbered-grid">
                @foreach ($items as $item)
                <div class="menu-numbered-item">
                    <span class="menu-item-counter">{{ str_pad($loop->index + 1, 2, '0', STR_PAD_LEFT) }}</span>
...
@else
    ...
    <section id="menu" class="section-wrap section-dark">
        <div class="section-inner">
            <div class="section-heading">
                <h2>Carta / Menú</h2>
                <div class="heading-line"></div>
            ...
            <div style="text-align:center;margin-top:2.5rem;">
                <a href="{{ route('site.fullpage', ['slug' => $site->slug, 'sectionType' => 'menu']) }}" class="services-more-btn">
                    Ver carta completa
                </a>
            </div>
            @endif
...
<style>
    ...
    /* ── Numbered layout ── */
    .menu-numbered-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 2px; }
    .menu-numbered-item { padding: 2rem 1.5rem; border: 1px solid var(--outline); background: var(--surface-lowest); }
    .menu-numbered-item:hover { background: var(--surface-low); }
    .menu-item-counter { display: block; font-size: .75rem; font-weight: 700; letter-spacing: .2em; color: var(--on-surface-muted); margin-bottom: 1rem; }
    .menu-item-name    { font-family: var(--font-heading); font-size: 1.15rem; font-weight: 700; color: var(--on-surface); margin-bottom: .5rem; }
    .menu-item-price   { color: var(--primary); font-weight: 700; font-size: 1rem; margin-bottom: .5rem; }
    .menu-item-desc    { color: var(--on-surface-muted); font-size: .85rem; line-height: 1.6; }
    @media (max-width: 1023px) { .menu-numbered-grid { grid-template-columns: repeat(2, 1fr); } }
    @media (max-width: 640px)  { .menu-numbered-grid { grid-template-columns: 1fr; } }
</style>
@endif
