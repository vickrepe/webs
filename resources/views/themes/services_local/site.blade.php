...
<head>
    ...
    @include('themes.services_local._head_styles', ['site' => $site, 'fontHeading' => $fontHeading, 'fontBody' => $fontBody])
</head>
<body>
...
@include('themes.services_local._nav')
...
<main>
    @foreach ($template['sections'] as $type => $sectionSchema)
        ...
        @include("themes.services_local.sections.{$type}", ['data' => $data, 'site' => $site])
    @endforeach
</main>

@include('themes.services_local._footer')
...
@if ($sections->has('whatsapp_cta') && $sections->get('whatsapp_cta')->active)
    ...
    @if (!empty($wa['phone']))
        <a class="wa-btn"
           href="https://wa.me/{{ preg_replace('/\D/', '', $wa['phone']) }}?text={{ urlencode($wa['message'] ?? '¡Hola! Necesito un presupuesto.') }}"
           target="_blank"
