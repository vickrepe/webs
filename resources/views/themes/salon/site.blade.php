...
<head>
    ...
    @include('themes.salon._head_styles', ['site' => $site, 'fontHeading' => $fontHeading, 'fontBody' => $fontBody])
</head>
<body>
...
<main>
    @foreach ($template['sections'] as $type => $sectionSchema)
        ...
        @include("themes.salon.sections.{$type}", ['data' => $data, 'site' => $site])
    @endforeach
</main>

@include('themes.salon._footer')
...
@if ($sections->has('whatsapp_cta') && $sections->get('whatsapp_cta')->active)
    ...
    @if (!empty($wa['phone']))
        <a class="wa-btn"
           href="https://wa.me/{{ preg_replace('/\D/', '', $wa['phone']) }}?text={{ urlencode($wa['message'] ?? '¡Hola! Quiero reservar.') }}"
           target="_blank"
